...
def get_Uniswap_v2_flashloan():
    ...
    columns_name = ['block_number', 'timestamp', 'tx_hash', 'transactionIndex', 'logIndex', 'sender', 'to', 'pair_contract_address', 'amount0_in', 'amount1_in', 'amount0_out', 'amount1_out', 'date']
    ...
    with Pool(num_processes) as pool:
        for result in tqdm(pool.imap(process_swap_event, range(len(swap_events))), total=len(swap_events)):
            if result is not None:
                if result[10] == 0:
                    amount = result[11]
                    smart_contract = w3.eth.contract(address=convert_to_checksum_address(result[7]), abi=uniswap_v2_abi)
                ...
                info = {
                    ...
                    'sender': result[5], 
                    'to': result[6], 
                    'pair_contract_address': result[7], 
                    'amount0_in': result[8], 
                    'amount1_in': result[9], 
