...
swap_events = vaex.open('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/uni-v2-sender_equal_to.hdf5')
...
def process_swap_event(i):
    result = Uniswap_v2_swap_or_flashloan(swap_events[i][2], AB.convert_to_checksum_address(swap_events[i][7]))
    if result == 1:
        return swap_events[i]
    else:
        return None

def get_Uniswap_v2_flashloan():
    Uniswap_v2_flashloan_logs = []
    ...
    with Pool(num_processes) as pool:
        for result in tqdm(pool.imap(process_swap_event, range(len(swap_events))), total=len(swap_events)):
            if result is not None:
                if result[10] == 0:
                    amount = result[11]
                    smart_contract = w3.eth.contract(address=convert_to_checksum_address(result[5]), abi=uniswap_v2_abi)
                    flashloan_token = smart_contract.functions.token1().call()
                else:
                    amount = result[10]
                    smart_contract = w3.eth.contract(address=convert_to_checksum_address(result[5]), abi=uniswap_v2_abi)
                    flashloan_token = smart_contract.functions.token0().call()
                info = {
                    'block_number': result[0],
                    'timestamp': result[1], 
                    'tx_hash': result[2], 
                    'transactionIndex': result[4], 
                    'logIndex': result[3], 
                    'sender': result[6], 
                    'to': result[7], 
                    'pair_contract_address': result[5], 
                    'amount0_in': result[8], 
                    'amount1_in': result[9], 
                    'amount0_out': result[10], 
                    'amount1_out': result[11], 
                    'date': timestamp_df.loc[timestamp_df['timestamp'] == result[1], 'date'].iloc[0],
                    'token': flashloan_token,
                    'amount': amount   
                }
                Uniswap_v2_flashloan_logs.append(info)

    Uniswap_v2_flashloan_logs_df = pd.DataFrame(Uniswap_v2_flashloan_logs).sort_values(by='timestamp').reset_index(drop=True)
    ...
    return Uniswap_v2_flashloan_logs_df, Uniswap_v2_flashloan_unique_df, Uniswap_v2_flashloan_count_df

Uniswap_v2_flashloan_logs_df, Uniswap_v2_flashloan_unique_df, Uniswap_v2_flashloan_count_df= get_Uniswap_v2_flashloan()