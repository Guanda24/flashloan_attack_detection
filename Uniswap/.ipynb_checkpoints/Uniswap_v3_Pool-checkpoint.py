...
step = 50000
v3_PoolCreated_logs = []
v3_PoolCreated_logs_simplified = []

for i in range(Uniswap_v3_start_block, w3_current_block + 1, step):
    v3_PoolCreated_logs = Uniswap_v3_smart_contract.events.PoolCreated().get_logs(fromBlock=i,toBlock=i+step-1)
    for j in range(len(v3_PoolCreated_logs)):
        info = {
            'token0': v3_PoolCreated_logs[j]['args']['token0'],
            'token1': v3_PoolCreated_logs[j]['args']['token1'],
            'fee': v3_PoolCreated_logs[j]['args']['fee'],
            'tickSpacing': v3_PoolCreated_logs[j]['args']['tickSpacing'],
            'pool': v3_PoolCreated_logs[j]['args']['pool'],
            'blockNumber': v3_PoolCreated_logs[j]['blockNumber'],
        }
        v3_PoolCreated_logs_simplified.append(info)

df = pd.DataFrame(v3_PoolCreated_logs_simplified)
df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/v3_Pool.csv', index=False)
