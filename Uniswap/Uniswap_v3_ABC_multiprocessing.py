...
manager = Manager()
Uniswap_v3_ABC_error_index = manager.list()

# File Location
Uniswap_v3_ABC_file = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC.csv'
Uniswap_v3_ABC_error_tx_cant_be_solved_file = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC_error_tx_cant_be_solved.csv'

try:
    Uniswap_v3_ABC = pd.read_csv(Uniswap_v3_ABC_file)
except Exception as e:
    print(f"File {Uniswap_v3_ABC_file} not found or cannot be loaded. Initializing a new one.")
    Uniswap_v3_ABC_columns = [
    ...
    print(f"File {Uniswap_v3_ABC_file} not found or cannot be loaded. Initializing a new one.")
    pd.DataFrame([], columns=Uniswap_v3_ABC_columns).to_csv(Uniswap_v3_ABC_file, index=False)
    Uniswap_v3_ABC = pd.DataFrame(columns=Uniswap_v3_ABC_columns)

try:
    Uniswap_v3_ABC_error_tx_cant_be_solved = pd.read_csv(Uniswap_v3_ABC_error_tx_cant_be_solved_file, header=None)[0].tolist()
except Exception as e:
    print(f"File {Uniswap_v3_ABC_error_tx_cant_be_solved_file} not found or cannot be loaded. Initializing a new one.")
    pd.DataFrame([]).to_csv(Uniswap_v3_ABC_error_tx_cant_be_solved_file, index=False, header=False)
    Uniswap_v3_ABC_error_tx_cant_be_solved = []

# File Lock Path
Uniswap_v3_ABC_lock_path = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC.lock'
Uniswap_v3_ABC_error_tx_cant_be_solved_lock_path = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC_error_tx_cant_be_solved.lock'
...
def process_transaction_with_default_abi(i):
    ...
    if tx_hash in Uniswap_v3_ABC['tx_hash'].values:
        return
    
    try:    
        block_number = Uniswap_v3_flashloan_unique['block_number'][i]
        logIndex = Uniswap_v3_flashloan_unique['logIndex'][i]
        transactionIndex = Uniswap_v3_flashloan_unique['transactionIndex'][i]
        date = Uniswap_v3_flashloan_unique['date'][i]
        from_address = w3.tracing.trace_transaction(tx_hash)[0]['action']['from'].lower()
        to_address = Uniswap_v3_flashloan_unique['recipient'][i].lower()


        # flashloan in usd
        flashloan_token_address = Uniswap_v3_flashloan_unique['token'][i].lower()
        flashloan_amount = Uniswap_v3_flashloan_unique['amount'][i]
        ...
        with FileLock(Uniswap_v3_ABC_lock_path):
            pd.DataFrame([entry]).to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC.csv', mode='a', index=False, header=False)
    except Exception as e:
        if tx_hash not in Uniswap_v3_ABC_error_tx_cant_be_solved:
            Uniswap_v3_ABC_error_index.append(i)       
        print(f"An error occurred: {e} at index {i}")
        
def process_error_transaction(i):
    error_index = Uniswap_v3_ABC_error_index[i]
    tx_hash = Uniswap_v3_flashloan_unique['tx_hash'][error_index]  
    
    if tx_hash in Uniswap_v3_ABC['tx_hash'].values:
        return
    
    try:
        block_number = Uniswap_v3_flashloan_unique['block_number'][error_index]
        logIndex = Uniswap_v3_flashloan_unique['logIndex'][error_index]
        transactionIndex = Uniswap_v3_flashloan_unique['transactionIndex'][error_index]
        date = Uniswap_v3_flashloan_unique['date'][error_index]
        from_address = w3.tracing.trace_transaction(tx_hash)[0]['action']['from'].lower()
        to_address = Uniswap_v3_flashloan_unique['recipient'][error_index].lower()
        ...
        if tx_hash not in Uniswap_v3_ABC_error_tx_cant_be_solved:
            with FileLock(Uniswap_v3_ABC_error_tx_cant_be_solved_lock_path):
                pd.DataFrame([[tx_hash]]).to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC_error_tx_cant_be_solved.csv', mode='a', index=False, header=False)
        print(f"Error cannot be solved: {e} at {error_index}")

# Process trasactions with default abi
num_processes = 32
with Pool(num_processes) as pool:
    list(tqdm(pool.imap_unordered(process_transaction_with_default_abi, range(len(Uniswap_v3_flashloan_unique))),
              desc='Processing transactions with default abi', total=len(Uniswap_v3_flashloan_unique)))   
    
# Process the error transactions by getting abi from etherscan
if Uniswap_v3_ABC_error_index:
    num_processes = 2
    with Pool(num_processes) as pool:
        list(tqdm(pool.imap_unordered(process_error_transaction, range(len(Uniswap_v3_ABC_error_index))),
                  desc='Processing transactions by getting abi from etherscan', total=len(Uniswap_v3_ABC_error_index)))
