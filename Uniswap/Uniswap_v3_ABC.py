...
try:
    ...
    Uniswap_v3_ABC_error_tx_cant_be_solved = manager.list()
...
def get_ABC_in_usd(tx_hash, from_address, to_address, ABC_simplified, date):
    ...
    for i in range(len(token_address_list)):
        ...
        if not token_price[(token_price['token.id'] == token_address) & (token_price['date'] == date)].empty:
            ...
            price = Decimal('NaN')
            if token_address not in Uniswap_v3_price_cannot_get:
                Uniswap_v3_price_cannot_get.append(token_address)
...
def get_flashloan_in_usd(flashloan_token_address, flashloan_amount, date):
    if not token_price[(token_price['token.id'] == flashloan_token_address) & (token_price['date'] == date)].empty:
        ...
        price = Decimal('NaN')
        if flashloan_token_address not in Uniswap_v3_price_cannot_get:
            Uniswap_v3_price_cannot_get.append(flashloan_token_address)
...
def process_transaction_with_default_abi(i):
    ...
    try:    
        ...
        to_address = Uniswap_v3_flashloan_unique['sender'][i].lower()
...
def process_error_transaction(i):
    try:
        ...
        from_address = w3.tracing.trace_transaction(tx_hash)[0]['action']['from'].lower()
        to_address = Uniswap_v3_flashloan_unique['sender'][error_index].lower()
        ...
        if tx_hash not in Uniswap_v3_ABC_error_tx_cant_be_solved:
            Uniswap_v3_ABC_error_tx_cant_be_solved.append(tx_hash)
...
print('initialise')
Uniswap_v3_ABC = list(Uniswap_v3_ABC)
Uniswap_v3_ABC_error_tx_cant_be_solved = list(Uniswap_v3_ABC_error_tx_cant_be_solved)
Uniswap_v3_price_cannot_get = list(Uniswap_v3_price_cannot_get)

print('to df')
Uniswap_v3_ABC_df = pd.DataFrame(Uniswap_v3_ABC)
Uniswap_v3_ABC_error_tx_cant_be_solved_df = pd.DataFrame(Uniswap_v3_ABC_error_tx_cant_be_solved)
Uniswap_v3_price_cannot_get_df = pd.DataFrame(Uniswap_v3_price_cannot_get)

print('to_csv')
Uniswap_v3_ABC_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC.csv', index=False)
Uniswap_v3_ABC_error_tx_cant_be_solved_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_ABC_error_tx_cant_be_solved.csv', index=False, header = False)
Uniswap_v3_price_cannot_get_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v3_price_cannot_get.csv', index=False, header = False)
