...
for i in range(Uniswap_v2_start_block, w3_current_block + 1, step):
    ...
    for j in range(len(Uniswap_v2_PairCreated_logs)):
        info = {
            'token0': Uniswap_v2_PairCreated_logs[j]['args']['token0'],
            'token1': Uniswap_v2_PairCreated_logs[j]['args']['token1'],
            'pair': Uniswap_v2_PairCreated_logs[j]['args']['pair'],
            'blockNumber': Uniswap_v2_PairCreated_logs[j]['blockNumber'],
        }
        Uniswap_v2_PairCreated_logs_simplified.append(info)
        Uniswap_v2_Pair.append(Uniswap_v2_PairCreated_logs[j]['args']['pair'])
        
df = pd.DataFrame(Uniswap_v2_PairCreated_logs_simplified)
df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Uniswap_v2_Pair.csv', index=False)