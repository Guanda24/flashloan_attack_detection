...
# File Location
Aave_v3_ABC_file = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC.csv'
Aave_v3_ABC_error_tx_cant_be_solved_file = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC_error_tx_cant_be_solved.csv'

try:
    Aave_v3_ABC = pd.read_csv(Aave_v3_ABC_file)
except Exception as e:
    print(f"File {Aave_v3_ABC_file} not found or cannot be loaded. Initializing a new one.")
    Aave_v3_ABC_columns = [
    ...
    print(f"File {Aave_v3_ABC_file} not found or cannot be loaded. Initializing a new one.")
    pd.DataFrame([], columns=Aave_v3_ABC_columns).to_csv(Aave_v3_ABC_file, index=False)
    Aave_v3_ABC = pd.DataFrame(columns=Aave_v3_ABC_columns)

try:
    Aave_v3_ABC_error_tx_cant_be_solved = pd.read_csv(Aave_v3_ABC_error_tx_cant_be_solved_file, header=None)[0].tolist()
except Exception as e:
    print(f"File {Aave_v3_ABC_error_tx_cant_be_solved_file} not found or cannot be loaded. Initializing a new one.")
    pd.DataFrame([]).to_csv(Aave_v3_ABC_error_tx_cant_be_solved_file, index=False, header=False)
    Aave_v3_ABC_error_tx_cant_be_solved = []

# File Lock Path
Aave_v3_ABC_lock_path = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC.lock'
Aave_v3_ABC_error_tx_cant_be_solved_lock_path = '/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC_error_tx_cant_be_solved.lock'
...
def process_transaction_with_default_abi(i):
    ...
    if tx_hash in Aave_v3_ABC['tx_hash'].values:
        return
    ...
    try:    
        ...
        with FileLock(Aave_v3_ABC_lock_path):
            pd.DataFrame([entry]).to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC.csv', mode='a', index=False, header=False)
    except Exception as e:
...
def process_error_transaction(i):
    ...
    try:
        ...
        if tx_hash not in Aave_v3_ABC_error_tx_cant_be_solved:
            with FileLock(Aave_v3_ABC_error_tx_cant_be_solved_lock_path):
                pd.DataFrame([[tx_hash]]).to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v3_ABC_error_tx_cant_be_solved.csv', mode='a', index=False, header=False)
