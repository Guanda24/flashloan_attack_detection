...
manager = Manager()
Aave_v2_ABC = manager.list()
Aave_v2_ABC_error_index = manager.list()
Aave_v2_price_cannot_get = manager.list()
...
try:
    Aave_v2_ABC_error_tx_cant_be_solved = manager.list(pd.read_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v2_ABC_error_tx_cant_be_solved.csv', header=None)[0].tolist())
except Exception as e:
    Aave_v2_ABC_error_tx_cant_be_solved = manager.list()
...
def get_ABC_in_usd(tx_hash, from_address, to_address, ABC_simplified, date):
    ...
    for i in range(len(token_address_list)):
        ...
        if not token_price[(token_price['token.id'] == token_address) & (token_price['date'] == date)].empty:
            ...
            price = Decimal('NaN')
            if token_address not in Aave_v2_price_cannot_get:
                Aave_v2_price_cannot_get.append(token_address)
...
def get_flashloan_in_usd(flashloan_token_address, flashloan_amount, date):
    if not token_price[(token_price['token.id'] == flashloan_token_address) & (token_price['date'] == date)].empty:
        ...
        price = Decimal('NaN')
        if flashloan_token_address not in Aave_v2_price_cannot_get:
            Aave_v2_price_cannot_get.append(flashloan_token_address)
...
def process_transaction_with_default_abi(i):
    tx_hash = Aave_v2_flashloan_unique['tx_hash'][i]
    # Pass current iteration if current tx_hash is marked as error:
    if tx_hash in Aave_v2_ABC_error_tx_cant_be_solved:
        return
    
    try:    
        block_number = Aave_v2_flashloan_unique['block_number'][i]
        logIndex = Aave_v2_flashloan_unique['logIndex'][i]
        transactionIndex = Aave_v2_flashloan_unique['transactionIndex'][i]
        date = Aave_v2_flashloan_unique['date'][i]
        from_address = w3.tracing.trace_transaction(tx_hash)[0]['action']['from'].lower()
        to_address = Aave_v2_flashloan_unique['recipient'][i].lower()

        # flashloan in usd
        flashloan_token_address = Aave_v2_flashloan_unique['token'][i].lower()
        flashloan_amount = Aave_v2_flashloan_unique['amount'][i]
...
def process_error_transaction(i):
    try:
        ...
        date = Aave_v2_flashloan_unique['date'][error_index]
        from_address = w3.tracing.trace_transaction(tx_hash)[0]['action']['from'].lower()
        to_address = Aave_v2_flashloan_unique['recipient'][error_index].lower()
        ...
        if tx_hash not in Aave_v2_ABC_error_tx_cant_be_solved:
            Aave_v2_ABC_error_tx_cant_be_solved.append(tx_hash)
...
print('initialise')
Aave_v2_ABC = list(Aave_v2_ABC)
Aave_v2_ABC_error_tx_cant_be_solved = list(Aave_v2_ABC_error_tx_cant_be_solved)
Aave_v2_price_cannot_get = list(Aave_v2_price_cannot_get)

print('to df')
Aave_v2_ABC_df = pd.DataFrame(Aave_v2_ABC)
Aave_v2_ABC_error_tx_cant_be_solved_df = pd.DataFrame(Aave_v2_ABC_error_tx_cant_be_solved)
Aave_v2_price_cannot_get_df = pd.DataFrame(Aave_v2_price_cannot_get)

print('to_csv')
Aave_v2_ABC_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v2_ABC.csv', index=False)
Aave_v2_ABC_error_tx_cant_be_solved_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v2_ABC_error_tx_cant_be_solved.csv', index=False, header = False)
Aave_v2_price_cannot_get_df.to_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v2_price_cannot_get.csv', index=False, header = False)
