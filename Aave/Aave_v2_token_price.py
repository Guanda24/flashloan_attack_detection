...
def collect_batch_data(token_id,data_name,query,start_time,end_time,save_path):
    ...
    while True:
        ...
        try:
            liq_calls = data["data"]["tokenDayDatas"]
...
df=pd.read_csv('/local/scratch/exported/MP_Defi_txs_TY_23/guanda/Aave_v2_unique_token.csv')
token_address=df['address'].tolist()

save_path = '/home/user/gzhao/Thesis/Price/' #change to your path
start_time=1606771548
end_time=1718259995
for token_id in tqdm(token_address):
    collect_batch_data(token_id,'Aave_v2',query, start_time, end_time, save_path)
    sleep(2)